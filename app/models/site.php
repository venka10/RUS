...
//		}
	...
	function calculateRuralityPoints($site_parm){
		...
		if(isset($site_parm['census_designations_id'])){
			$cd = $this->CensusDesignation->read(null,$site_parm['census_designations_id']);
			$site_parm['CensusDesignation'] = $cd['CensusDesignation'];
			//pr($site_parm);
			switch($site_parm['CensusDesignation']['cd']){
				case "Census Rural":
					return 45;
				case "Urban Cluster":
					if($site_parm['rwf_census_population'] <= 5000){
						return 45;
					}elseif ($site_parm['rwf_census_population'] > 5000 && $site_parm['rwf_census_population'] <= 10000){
						return 30;
					}elseif ($site_parm['rwf_census_population'] > 10000 && $site_parm['rwf_census_population'] <= 20000){
						return 15;
					}else{
						return 0;
					}
